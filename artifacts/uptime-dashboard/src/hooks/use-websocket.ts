...
import type { MonitorWithStatus, Alert } from '@workspace/api-client-react';
import { toast } from 'sonner';

type WSMessage = 
  | { type: 'check_result'; monitor: MonitorWithStatus }
  | { type: 'alert'; alert: Alert & { eventType: string; message: string; monitorId: number } };

export function useMonitorWebSocket() {
  const queryClient = useQueryClient();
  const wsRef = useRef<WebSocket | null>(null);
  const reconnectTimeoutRef = useRef<NodeJS.Timeout>();
  const batchUpdatesRef = useRef<Map<number, MonitorWithStatus>>(new Map());
          ...
          if (data.type === 'check_result') {
            ...
            if (!batchTimerRef.current) {
              batchTimerRef.current = setTimeout(() => {
                ...
                queryClient.setQueryData<MonitorWithStatus[]>(
