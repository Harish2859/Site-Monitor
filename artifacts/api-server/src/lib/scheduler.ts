...
// In-memory state
const lastCheckedAt = new Map<number, Date>();
const inFlight = new Set<number>();
const monitorStatus = new Map<number, "UP" | "DOWN">();
const rollingResponseTimes = new Map<number, number[]>();
...
async function checkMonitor(monitor: Monitor): Promise<void> {
  ...
  try {
    const uptimeRow = await db.execute<{ up_count: string; total_count: string }>(sql`
